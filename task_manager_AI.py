...
# Function - Register a user - username and password 
...
def reg_user():
    ...
    # Prevent duplication of user names when registering a user
        ...
        for line in file:
            ...
            if new_user == user_check:
                ...
                duplicate_found = True  # prevents duplicates username to be added
            ...
            # Checks passwords and confirms registration of the new user to the task text file
                ...
                if new_password == input("Confirm password: "):
                    file.write("\n" + new_user + ", ") # Writes back to user txt file
...
# If "va" is chosen, print all tasks for all users as output
def view_all():
...
def view_mine():
    ...
    if choice == -1: # user selects -1 to return to menu
        menu()
    else:
        ...
        if edit_type == "a":    # user edits task as completed
            user_tasks[choice - 1][5] = "Yes\n"

        elif edit_type == "b":  # user edits task as incomplete
            user_tasks[choice - 1][5] = "No\n"

        elif edit_type == "c":
            new_date = input("Enter the new due date")    # user edits due date with a new date
            user_tasks[choice - 1][4] = f"{new_date}"
  
        elif edit_type == "d":  # user edit the assigned task manager
            new_task_manager = input("Enter the new task manager")
            user_tasks[choice - 1][0] = f"{new_task_manager}"

        print(user_tasks)
        other_file = open("tasks.txt", "w")    # open file to write back edited tasks
...
# If "ds" is chosen, display stats summary for users and tasks
def disp_stats():
    if username == "admin":
...
# Function - Generate reports via two overview files (task & user)    
    ...
    with open("task_overview.txt", "w") as write_task, open("user_overview.txt", "w") as write_user:
        with open("tasks.txt", "r+") as task_file, open("user.txt", "r+") as user_file:
            ...
            for line in lines:
                ...
                if is_completed == "Yes":   # algo to determine completed tasks
                    total_completed += 1
                ...
                if is_completed == "No" and is_overdue: # algo to determine no of overdue tasks
